...
def configure_chinese_fonts():
    """Configure matplotlib for Chinese character support with robust fallback chain"""
    try:
        ...
        with open(os.path.join(fontconfig_dir, '99-chinese.conf'), 'w') as f:
...
<fontconfig>
    ...
    <match target="pattern">
        ...
        # Write fontconfig configuration
        with open(os.path.join(fontconfig_dir, 'fonts.conf'), 'w') as f:
            f.write("""<?xml version="1.0"?>
<!DOCTYPE fontconfig SYSTEM "fonts.dtd">
<fontconfig>
    <match target="pattern">
        ...
        # First try to load fonts from absolute paths
        font_paths = [
            ...
            '/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf',  # Add DejaVu Sans
            '/usr/share/fonts/truetype/liberation/LiberationSans-Regular.ttf',  # Add Liberation Sans
            ...
            os.path.expanduser('~/.local/share/fonts/WenQuanYi.ttf'),
            os.path.expanduser('~/.fonts/SimHei.ttf')
        ]
        
        # Collect working fonts
        working_fonts = []
        
        # Try loading fonts from paths first
        for font_path in font_paths:
            if os.path.exists(font_path):
                try:
                    ...
                    logger.info(f"Successfully loaded font from path: {font_path}")
                except Exception as e:
                    logger.warning(f"Failed to load font from {font_path}: {e}")
        
        # Then try font names as fallback
        fallback_fonts = [
            'WenQuanYi Micro Hei',
            'Noto Sans CJK SC',
            'AR PL UMing CN',
        ...
        # Test each font with comprehensive characters
        test_str = "测试中文字体渲染效果123ABC"
        
        for font in fallback_fonts:
            try:
                # Create test figure
                fig = plt.figure(figsize=(1, 1))
                plt.text(0.1, 0.1, test_str, fontsize=12, family=font)
                plt.close(fig)
                
                if font not in working_fonts:
                    working_fonts.append(font)
                    logger.info(f"Successfully loaded font: {font}")
            except Exception as e:
                logger.warning(f"Failed to load font {font}: {e}")
                continue
        
        if working_fonts:
            # Configure font settings with working fonts
            # Find first working Chinese font
            chinese_font = None
            for font_path in font_paths:
                if os.path.exists(font_path):
                    try:
            ...
            if not chinese_font:
                logger.error("No working Chinese fonts found")
                return False
                
            # Configure matplotlib with direct font file
            font_prop = mpl.font_manager.FontProperties(fname=chinese_font)
            font_name = font_prop.get_name()
            ...
            logger.info("Successfully configured Chinese fonts")
            return True
            ...
            logger.info(f"Successfully configured Chinese fonts: {', '.join(working_fonts)}")
            return True
        else:
            logger.error("No working Chinese fonts found")
            return False
        ...
        logger.error(f"Failed to configure Chinese fonts: {e}")
        return False
        ...
        # Disable LaTeX and set custom math font
        plt.rcParams['text.usetex'] = False
        plt.rcParams['mathtext.fontset'] = 'custom'
        plt.rcParams['mathtext.rm'] = 'WenQuanYi Micro Hei'
        plt.rcParams['mathtext.it'] = 'WenQuanYi Micro Hei'
        plt.rcParams['mathtext.bf'] = 'WenQuanYi Micro Hei'
        
        # Force matplotlib to use system-wide Chinese fonts
        font_paths = [
            '/usr/share/fonts/truetype/wqy/wqy-microhei.ttc',
            '/usr/share/fonts/opentype/noto/NotoSansCJK-Regular.ttc',
            '/usr/share/fonts/truetype/wqy/wqy-zenhei.ttc',
        ...
        for font_path in font_paths:
            if os.path.exists(font_path):
                try:
                    # Add font file to matplotlib's font manager
                    font_prop = mpl.font_manager.FontProperties(fname=font_path)
                    mpl.font_manager.fontManager.addfont(font_path)
                    ...
                    # Set font for each text element
                    ...
                    # Set font for text.Text
                    plt.rcParams['text.color'] = 'black'
                    plt.rcParams['text.usetex'] = False
                    plt.rcParams['text.antialiased'] = True
                    
                    # Set font for savefig
                    ...
                    logger.info(f"Successfully configured Chinese font: {font_prop.get_name()}")
                    success = True
                except Exception as e:
                    logger.warning(f"Failed to configure font {font_path}: {e}")
                    continue
        ...
        # Force matplotlib to use SimHei font
        font_path = os.path.expanduser('~/.local/share/fonts/SimHei.ttf')
        if os.path.exists(font_path):
            font_prop = mpl.font_manager.FontProperties(fname=font_path)
            plt.rcParams['font.sans-serif'] = ['SimHei']
            mpl.rcParams['font.family'] = font_prop.get_name()
            logger.info(f"Successfully configured Chinese font: SimHei ({font_path})")
            return True
        ...
        # Test each font in the fallback list
        for font in plt.rcParams['font.sans-serif']:
            try:
                font_path = mpl.font_manager.findfont(font)
                if font_path:
                    plt.rcParams['font.sans-serif'] = [font]
                    logger.info(f"Successfully configured Chinese font: {font} ({font_path})")
                    return True
            except Exception as e:
                logger.warning(f"Failed to use font {font}: {e}")
                continue
        
        logger.error("Failed to find any suitable Chinese fonts")
        return False
...
class EnhancedVisualizerV2:
    ...
    def __init__(self, output_dir="organized_results"):
        ...
        self._chapter = 3
        self._category = 'performance'
        self._subcategory = 'analysis'
    ...
    def set_visualization_params(self, chapter: int = None, category: str = None, subcategory: str = None):
    ...
    def save_visualization(self, filepath: str, chapter: int = None, category: str = None, subcategory: str = None) -> str:
        """Save visualization with proper categorization"""
        if not filepath or not os.path.exists(filepath):
            return ""
            
        # Use provided parameters or instance defaults
        ch = chapter if chapter is not None else self._chapter
        cat = category if category is not None else self._category
        subcat = subcategory if subcategory is not None else self._subcategory
            
        # Create chapter-specific directory
        base_dir = os.path.join(self.output_dir, f"chapter{ch}", cat)
        if subcat:
            base_dir = os.path.join(base_dir, subcat)
        ...
        logger.info(f"Generated visualization {self.visualization_count} in chapter {ch}/{cat}: {new_filepath}")
    ...
    def plot_metrics_comparison(self, metrics_dict: Dict[str, Dict[str, float]], title: str,
        ...
        try:
            ...
            filename = sanitize_filename(title, self._chapter, self._category, self._subcategory)
            filepath = os.path.join(self.temp_dir, filename)
            plt.savefig(filepath, dpi=300, bbox_inches='tight')
            plt.close()
    ...
    def plot_scenario_comparison(self, scenario_results: Dict[str, Dict[str, Dict[str, float]]], title: str,
                                  chapter: int = None, category: str = None, subcategory: str = None) -> str:
    ...
    def plot_violin(self, data: Dict[str, List[float]], title: str,
                       chapter: int = 3, category: str = 'performance',
                       subcategory: str = 'analysis') -> str:
    ...
    def plot_box(self, data: Dict[str, List[float]], title: str,
                    chapter: int = 3, category: str = 'performance',
                    subcategory: str = 'analysis') -> str:
    ...
    def plot_parallel_coordinates(self, data: Dict[str, Dict[str, float]], title: str,
                                    chapter: int = 3, category: str = 'performance',
                                    subcategory: str = 'analysis') -> str:
    ...
    def plot_path_density(self, paths: Dict[str, np.ndarray], title: str,
                           chapter: int = 3, category: str = 'performance',
                           subcategory: str = 'analysis') -> str:
    ...
    def plot_performance_radar(self, data: Dict[str, Dict[str, float]], title: str,
                                chapter: int = 3, category: str = 'performance',
                                subcategory: str = 'comparison') -> str:
    ...
    def plot_success_rate_by_scenario(self, data: Dict[str, Dict[str, Dict[str, float]]], title: str,
                                      chapter: int = 3, category: str = 'performance',
                                      subcategory: str = 'metrics') -> str:
    ...
    def plot_path_complexity(self, paths: Dict[str, np.ndarray], title: str,
                               chapter: int = 3, category: str = 'performance',
                               subcategory: str = 'analysis') -> str:
        """Plot path complexity analysis"""
        try:
            filepath = self._plot_path_complexity(paths, title, chapter, category, subcategory)
            return self.save_visualization(filepath, chapter, category, subcategory)
        except Exception as e:
            logger.error(f"Failed to create path complexity plot: {e}")
            return ""
        
    def _plot_path_complexity(self, paths: Dict[str, np.ndarray], title: str,
                                chapter: int = 3, category: str = 'performance',
                                subcategory: str = 'analysis') -> str:
    ...
    def plot_performance_metrics_3d(self, data: Dict[str, Dict[str, float]], title: str,
                                     chapter: int = 3, category: str = 'performance',
                                     subcategory: str = 'metrics') -> str:
    ...
    def plot_energy_efficiency(self, paths: Dict[str, np.ndarray], title: str,
                                chapter: int = 3, category: str = 'performance',
                                subcategory: str = 'analysis') -> str:
    ...
    def plot_obstacle_avoidance(self, paths: Dict[str, np.ndarray], env: Any, title: str,
                                  chapter: int = 3, category: str = 'performance',
                                  subcategory: str = 'analysis') -> str:
    ...
    def plot_convergence_speed(self, data: Dict[str, List[float]], iterations: List[int], title: str,
                                 chapter: int = 3, category: str = 'performance',
                                 subcategory: str = 'analysis') -> str:
    ...
    def plot_parameter_sensitivity(self, data: Dict[str, Dict[str, List[float]]], params: Dict[str, List[float]], title: str,
                                   chapter: int = 3, category: str = 'ablation',
                                   subcategory: str = 'components') -> str:
    ...
    def plot_multi_objective(self, data: Dict[str, Dict[str, float]], objectives: List[str], title: str,
                              chapter: int = 3, category: str = 'performance',
                              subcategory: str = 'analysis') -> str:
    ...
    def plot_real_time_performance(self, data: Dict[str, Dict[str, List[float]]], title: str,
                                    chapter: int = 3, category: str = 'performance',
                                    subcategory: str = 'analysis') -> str:
    ...
    def plot_path_smoothness(self, paths: Dict[str, np.ndarray], title: str,
                               chapter: int = 3, category: str = 'performance',
                               subcategory: str = 'analysis') -> str:
    ...
    def plot_path_efficiency(self, paths: Dict[str, np.ndarray], title: str,
                               chapter: int = 3, category: str = 'performance',
                               subcategory: str = 'analysis') -> str:
    ...
    def plot_dynamic_obstacle_analysis(self, data: Dict[str, Dict[str, List[float]]], title: str,
                                       chapter: int = 4, category: str = 'performance',
                                       subcategory: str = 'analysis') -> str:
    ...
    def plot_success_rate_analysis(self, data: Dict[str, Dict[str, float]], scenarios: List[str], title: str,
                                    chapter: int = 3, category: str = 'performance',
                                    subcategory: str = 'metrics') -> str:
    ...
    def plot_computational_resources(self, data: Dict[str, Dict[str, List[float]]], title: str,
                                     chapter: int = 3, category: str = 'performance',
                                     subcategory: str = 'metrics') -> str:
    ...
    def plot_path_quality(self, paths: Dict[str, np.ndarray], metrics: Dict[str, Dict[str, float]], title: str,
                            chapter: int = 3, category: str = 'performance',
                            subcategory: str = 'metrics') -> str:
    ...
    def plot_environmental_impact(self, paths: Dict[str, np.ndarray], env: Any, title: str,
                                   chapter: int = 3, category: str = 'performance',
                                   subcategory: str = 'analysis') -> str:
    ...
    def plot_multi_drone_coordination(self, paths: Dict[str, List[np.ndarray]], title: str,
                                       chapter: int = 5, category: str = 'performance',
                                       subcategory: str = 'analysis') -> str:
    ...
    def plot_robustness_analysis(self, data: Dict[str, Dict[str, List[float]]], title: str,
                                  chapter: int = 3, category: str = 'performance',
                                  subcategory: str = 'analysis') -> str:
    ...
    def plot_scalability_analysis(self, data: Dict[str, Dict[str, List[float]]], title: str,
                                   chapter: int = None, category: str = None, subcategory: str = None) -> str:
        ...
        try:
            ...
            # 1. Computation time vs number of drones
            for name, metrics in data.items():
                ax1.plot(metrics['num_drones'], metrics['computation_time'], 
                        marker='o', label=name)
            ax1.set_xlabel('无人机数量')
            ax1.set_ylabel('计算时间 (s)')
            ax1.set_title('计算时间可扩展性')
            ax1.legend()
            ax1.grid(True, alpha=0.3)
            
            # 2. Communication overhead vs number of drones
            for name, metrics in data.items():
                ax2.plot(metrics['num_drones'], metrics['communication_overhead'], 
                        marker='o', label=name)
            ax2.set_xlabel('无人机数量')
            ax2.set_ylabel('通信开销 (MB/s)')
            ax2.set_title('通信开销可扩展性')
            ax2.legend()
            ax2.grid(True, alpha=0.3)
            
            # 3. Success rate vs number of drones
            for name, metrics in data.items():
                ax3.plot(metrics['num_drones'], metrics['success_rate'], 
                        marker='o', label=name)
            ax3.set_xlabel('无人机数量')
            ax3.set_ylabel('成功率')
            ax3.set_title('成功率可扩展性')
            ax3.legend()
    ...
    def plot_adaptation_analysis(self, data: Dict[str, Dict[str, List[float]]], title: str,
                                  chapter: int = 3, category: str = 'performance',
                                  subcategory: str = 'analysis') -> str:
    ...
    def plot_learning_curves(self, data: Dict[str, Dict[str, List[float]]], title: str,
                              chapter: int = 3, category: str = 'performance',
                              subcategory: str = 'metrics') -> str:
    ...
    def plot_optimization_landscape(self, data: Dict[str, Dict[str, List[float]]], title: str,
                                    chapter: int = 3, category: str = 'performance',
                                    subcategory: str = 'analysis') -> str:
    ...
    def plot_training_progress(self, data: Dict[str, Dict[str, List[float]]], title: str,
                                chapter: int = None, category: str = None, subcategory: str = None) -> str:
        ...
        try:
            ...
            # 1. Loss curves
            for name, metrics in data.items():
                ax1.plot(metrics['epochs'], metrics['loss'], label=f'{name} 损失', marker='o', markersize=2)
            ax1.set_xlabel('训练轮次')
            ax1.set_ylabel('损失值')
            ...
            # 2. Formation accuracy or regular accuracy curves
            for name, metrics in data.items():
                if 'formation_accuracy' in metrics:
                    ax2.plot(metrics['epochs'], metrics['formation_accuracy'], 
                            label=f'{name} 编队准确率', marker='o', markersize=2)
                elif 'accuracy' in metrics:
                    ax2.plot(metrics['epochs'], metrics['accuracy'], 
                            label=f'{name} 准确率', marker='o', markersize=2)
            ax2.set_xlabel('训练轮次')
            ax2.set_ylabel('准确率')
    ...
    def plot_model_architecture(self, data: Dict[str, Dict[str, Any]], title: str,
                                chapter: int = 3, category: str = 'model',
                                subcategory: str = 'architecture') -> str:
        ...
        try:
            ...
            types = [info['type'] for info in data.values()]
            
            # Create layer visualization
            y_positions = np.arange(len(layers))
            plt.barh(y_positions, sizes, alpha=0.3)
            
            # Add layer labels
            for i, (layer, size, type_info) in enumerate(zip(layers, sizes, types)):
                plt.text(-10, i, layer, ha='right', va='center')
                plt.text(size/2, i, f'{type_info}\n({size} units)', 
                        ha='center', va='center')
            ...
            for i in range(len(layers)-1):
                plt.annotate('', xy=(sizes[i+1]/2, y_positions[i+1]), 
                           xytext=(sizes[i]/2, y_positions[i]),
    ...
    def plot_feature_importance(self, data: Dict[str, float], title: str,
                                chapter: int = 3, category: str = 'model',
                                subcategory: str = 'features') -> str:
    ...
    def plot_decision_boundary(self, data: Dict[str, Dict[str, List[float]]], title: str,
                                chapter: int = 3, category: str = 'performance',
                                subcategory: str = 'analysis') -> str:
    ...
    def plot_hyperparameter_analysis(self, data: Dict[str, Dict[str, List[float]]], title: str,
                                     chapter: int = None, category: str = None, subcategory: str = None) -> str:
    ...
    def plot_error_distribution(self, data: Dict[str, Dict[str, List[float]]], title: str,
                                  chapter: int = 3, category: str = 'performance',
                                  subcategory: str = 'metrics') -> str:
    ...
    def plot_ensemble_performance(self, data: Dict[str, Dict[str, List[float]]], title: str,
                                  chapter: int = 3, category: str = 'performance',
                                  subcategory: str = 'comparison') -> str:
    ...
    def plot_training_dynamics(self, data: Dict[str, Dict[str, List[float]]], title: str,
                                chapter: int = 3, category: str = 'performance',
                                subcategory: str = 'metrics') -> str:
    ...
    def plot_model_comparison(self, data: Dict[str, Any], title: str,
                                chapter: int = 3, category: str = 'model',
                                subcategory: str = 'comparison') -> str:
        """Plot model comparison using bar charts"""
        try:
    ...
    def plot_prediction_accuracy(self, data: Dict[str, Dict[str, List[float]]], title: str,
                               chapter: int = None, category: str = None, subcategory: str = None) -> str:
        ...
        try:
            ...
            for model_name, metrics in data.items():
                plt.plot(metrics['timesteps'], metrics['accuracy'], label=model_name)
            ...
            logger.error(f"Failed to create prediction accuracy plot: {e}")
            return ""
    ...
    def plot_dynamic_obstacle_analysis(self, data: Dict[str, Dict[str, List[float]]], title: str,
        ...
        try:
            ...
            # Plot distances
            for name, metrics in data.items():
                ax1.hist(metrics['distances'], bins=20, alpha=0.5, label=name)
            ax1.set_xlabel('避障距离 (m)')
            ax1.set_ylabel('频次')
            ax1.set_title('避障距离分布')
            ax1.legend()
            
            # Plot velocities
            for name, metrics in data.items():
                ax2.hist(metrics['velocities'], bins=20, alpha=0.5, label=name)
            ax2.set_xlabel('相对速度 (m/s)')
            ax2.set_ylabel('频次')
            ax2.set_title('相对速度分布')
            ax2.legend()
            ...
            logger.error(f"Failed to create dynamic obstacle analysis: {e}")
            return ""
    ...
    def plot_real_time_performance(self, data: Dict[str, Dict[str, List[float]]], title: str,
        ...
        try:
            ...
            # Processing time
            for name, metrics in data.items():
                ax1.plot(metrics['processing_time'], label=name)
            ax1.set_xlabel('时间步')
            ax1.set_ylabel('处理时间 (ms)')
            ax1.set_title('处理时间分析')
            ax1.legend()
            ax1.grid(True)
            
            # Prediction time
            for name, metrics in data.items():
                ax2.plot(metrics['prediction_time'], label=name)
            ax2.set_xlabel('时间步')
            ax2.set_ylabel('预测时间 (ms)')
            ax2.set_title('预测时间分析')
            ax2.legend()
            ax2.grid(True)
            
            # Planning time
            for name, metrics in data.items():
                ax3.plot(metrics['planning_time'], label=name)
            ax3.set_xlabel('时间步')
            ax3.set_ylabel('规划时间 (ms)')
            ax3.set_title('规划时间分析')
            ax3.legend()
            ax3.grid(True)
            ...
            logger.error(f"Failed to create real-time performance analysis: {e}")
            return ""

    def plot_formation_stability(self, data: Dict[str, Dict[str, List[float]]], title: str,
                                   chapter: int = None, category: str = None, subcategory: str = None) -> str:
        """Plot formation stability analysis"""
        try:
            plt.figure(figsize=(10, 6))
            
            for model_name, metrics in data.items():
                plt.plot(metrics['timesteps'], metrics['formation_error'], label=model_name)
            ...
            logger.error(f"Failed to create formation stability plot: {e}")
            return ""
            
    def plot_inter_drone_distances(self, data: Dict[str, Dict[str, List[float]]], title: str,
                                 chapter: int = None, category: str = None, subcategory: str = None) -> str:
        """Plot inter-drone distance analysis"""
        try:
            fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(15, 6))
            ...
            for name, metrics in data.items():
                ax1.hist(metrics['distances'], bins=20, alpha=0.5, label=name)
            ax1.set_xlabel('无人机间距 (m)')
            ax1.set_ylabel('频次')
            ax1.set_title('无人机间距分布')
            ax1.legend()
            ...
            logger.error(f"Failed to create inter-drone distance analysis: {e}")
            return ""
            
    def plot_communication_analysis(self, data: Dict[str, Dict[str, List[float]]], title: str,
                                    chapter: int = None, category: str = None, subcategory: str = None) -> str:
        """Plot communication performance analysis"""
        try:
            fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(15, 6))
            
            # Plot message count
            for name, metrics in data.items():
                ax1.hist(metrics['message_count'], bins=20, alpha=0.5, label=name)
            ax1.set_xlabel('消息数量')
            ax1.set_ylabel('频次')
            ax1.set_title('通信消息分布')
            ax1.legend()
            
            # Plot bandwidth usage
            for name, metrics in data.items():
                ax2.hist(metrics['bandwidth'], bins=20, alpha=0.5, label=name)
            ax2.set_xlabel('带宽使用 (MB/s)')
            ax2.set_ylabel('频次')
            ax2.set_title('带宽使用分布')
            ax2.legend()
            ...
            logger.error(f"Failed to create communication analysis: {e}")
            return ""
