import numpy as np
from .neural_ahpp import NeuralAHPP
...
class NeuralAHPPImproved(NeuralAHPP):
    ...
    def initialize_model(self):
        ...
        try:
            ...
            inputs = layers.Input(shape=(9,))  # state(6) + obstacle_features(3)
            ...
            x = layers.Dense(128, activation='relu')(x)
            x = layers.BatchNormalization()(x)
            x = layers.Dense(128, activation=None)(x)  # Match dimensions
            x = layers.Add()([x, skip])
            ...
            model.compile(optimizer=optimizer, 
                        loss=losses.Huber(),  # More robust to outliers
                        metrics=['mae'])
    ...
    def plan_path(self, start, goal, max_steps=1000):
