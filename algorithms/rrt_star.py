...
class RRTStar:
    ...
    def plan(self, start: np.ndarray, goal: np.ndarray, timeout: float = 5.0) -> Optional[np.ndarray]:
        """Plan a path from start to goal with timeout"""
        import time
        ...
        try:
            ...
            for i in range(self.max_iter):
                ...
                if time.time() - start_time > timeout:
                    logger.warning(f"RRT* planning timed out after {timeout} seconds")
                    return best_path
                ...
                if self._is_collision_free(nearest_node, new_node):
                    ...
                    # Check if goal is reached or we found a better path
                    ...
                    if dist_to_goal < best_dist_to_goal:
                        ...
                        if dist_to_goal < self.step_size:
                            ...
                            if path is not None:
                                best_path = self._smooth_path(path)
                                if dist_to_goal < 0.1:  # Early termination if very close
                                    return best_path
    ...
    def _find_nearest(self, point: np.ndarray) -> int:
        """Find nearest node index using vectorized operations"""
        nodes_array = np.array(self.nodes)
        distances = np.linalg.norm(nodes_array - point, axis=1)
        return np.argmin(distances)
        
    def _find_nearby(self, point: np.ndarray) -> List[int]:
        """Find nearby nodes within dynamic radius using vectorized operations"""
        try:
            n = len(self.nodes)
            ...
            nodes_array = np.array(self.nodes, dtype=np.float64)
